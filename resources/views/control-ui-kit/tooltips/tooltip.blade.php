@php
    ...
    [$axis, $near, $far, $viewport, $extent, $nudge] = $isHorizontal
        ? ['top', 'b.top', 'b.bottom', 'window.innerHeight', 'b.height', 'translateY']
        : ['left', 'b.left', 'b.right', 'window.innerWidth', 'b.width', 'translateX'];
@endphp
<div class="{{ $wrapper }}"
     x-data="{
         open: false,
         ...
         show(el) {
             ...
             this.nudge = 0;
             this.open = true;
             this.$nextTick(this.constrain.bind(this));
         },
         constrain() {
             if (! this.tip) { return; }
             const b = this.tip.getBoundingClientRect(), edge = 8;
             if (! b.width) { return; }
             const shift = Math.max(edge - {{ $near }}, 0) - Math.max({{ $far }} - {{ $viewport }} + edge, 0);
             const limit = Math.max({{ $extent }} / 2 - edge, 0);
             this.{{ $axis }} += shift;
             this.nudge = Math.min(Math.max(-shift, -limit), limit);
         },
         hide() {
             this.open = false;
         }
    ...
    <template x-teleport="body">
        <div x-show="open"
             x-init="tip = $el"
             :style="`position:fixed;top:0;left:0;transform:translate(${left}px, ${top}px) {{ $transform }}`"
            ...
            @if ($position === 'bottom')
                <div class="w-0 h-0 border-l-[5px] border-l-transparent border-r-[5px] border-r-transparent border-b-[5px]"
                     :style="`border-bottom-color: {{ $arrow }};transform:{{ $nudge }}(${nudge}px)`"></div>
            @elseif ($position === 'right')
                <div class="w-0 h-0 border-t-[5px] border-t-transparent border-b-[5px] border-b-transparent border-r-[5px]"
                     :style="`border-right-color: {{ $arrow }};transform:{{ $nudge }}(${nudge}px)`"></div>
            @endif
            ...
            @if ($position === 'left')
                <div class="w-0 h-0 border-t-[5px] border-t-transparent border-b-[5px] border-b-transparent border-l-[5px]"
                     :style="`border-left-color: {{ $arrow }};transform:{{ $nudge }}(${nudge}px)`"></div>
            @elseif ($position !== 'bottom' && $position !== 'right')
                <div class="w-0 h-0 border-l-[5px] border-l-transparent border-r-[5px] border-r-transparent border-t-[5px]"
                     :style="`border-top-color: {{ $arrow }};transform:{{ $nudge }}(${nudge}px)`"></div>
            @endif
        </div>
    </template>
</div>

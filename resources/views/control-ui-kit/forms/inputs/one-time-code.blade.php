...
<div
    ...
    <fieldset class="{{ trim("fs-{$name}-otc min-w-0 {$fieldset}") }}">
        @for($i = 1; $i <= $digits; $i++)
            <input id="{{ $name }}-{{ $i }}" data-digit="{{ $i }}" x-model="digit_{{ $i }}" type="number" {{ $attributes->whereDoesntStartWith(['class', 'x-model', 'wire:model'])->merge($basicClasses())->class('min-w-0') }} pattern="[0-9]*" min="0" max="9" maxlength="1" @if($requiredInput) required @endif />
        @endfor
    </fieldset>

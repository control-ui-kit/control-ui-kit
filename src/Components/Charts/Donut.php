...
class Donut extends Component
{
    ...
    protected string $donutSegment = 'charts.donut.segment';
    protected string $donutAnimation = 'charts.donut.animation';
    protected string $donutLayout = 'charts.donut.layout';
    protected string $donutConfig = 'charts.donut';
    ...
    public function __construct(
        ...
        $this->values = $values;
        $this->labels = $labels;
        $this->data = $this->resolveData($data, $values, $labels);

        $this->cutout = $this->style($this->donutConfig, 'cutout', $cutout);

        $this->colors = $this->getColours($colors);
        $this->hoverColors = $this->getHoverColours();

        $this->segmentBorderColor = $this->style($this->donutSegment, 'border-color', $segmentBorderColor);
        $this->segmentBorderWidth = $this->style($this->donutSegment, 'border-width', $segmentBorderWidth);
        $this->segmentHoverOffset = $this->style($this->donutSegment, 'hover-offset', $segmentHoverOffset);
        $this->animationDuration = $this->style($this->donutAnimation, 'duration', $animationDuration);
        $this->animationEasing = $this->style($this->donutAnimation, 'easing', $animationEasing);
        $this->animationRotate = $this->style($this->donutAnimation, 'animate-rotate', $animationRotate);
        $this->animationScale = $this->style($this->donutAnimation, 'animate-scale', $animationScale);
        $this->layoutPadding = $this->style($this->donutLayout, 'padding', $layoutPadding);
    ...
    private function resolveData(?array $data, ?array $values, ?array $labels): array
    {
        if (is_array($data)) {
            return $data;
        }

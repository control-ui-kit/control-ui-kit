...
use ControlUIKit\Components\Modals\Action;
...
class ActionTest extends ComponentTestCase
{
        ...
        $expected = <<<'HTML'
            <div x-data="{ show: false, loading: false, focusables() { // All focusable element types... let selector = 'a, button, input, textarea, select, details, [tabindex]:not([tabindex=\'-1\'])' return [...$el.querySelectorAll(selector)] // All non-disabled elements... .filter(el =>
                ...
                <div x-show="show" class="panel scroll-body" :class="{ [maxWidth]: true }" role="dialog" aria-modal="true" tabindex="-1" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
                    <form method="POST" action="/items/1" class="form">
                        <input type="hidden" name="_token" value="" autocomplete="off">
                        <div class="body text-sm leading-6"></div>
                        <div class="footer">
    ...
    public function an_action_modal_can_be_rendered_with_ajax_action(): void
    {
        ...
        $expected = <<<'HTML'
            <div x-data="{ show: false, loading: false, focusables() { // All focusable element types... let selector = 'a, button, input, textarea, select, details, [tabindex]:not([tabindex=\'-1\'])' return [...$el.querySelectorAll(selector)] // All non-disabled elements... .filter(el =>
                ! el.hasAttribute('disabled')) }, firstFocusable() { return this.focusables()[0] }, lastFocusable() { return this.focusables().slice(-1)[0] }, nextFocusable() { return this.focusables()[this.nextFocusableIndex()] || this.firstFocusable() }, prevFocusable() { return this.focusables()[this.prevFocusableIndex()] || this.lastFocusable() }, nextFocusableIndex() { return (this.focusables().indexOf(document.activeElement) + 1) % (this.focusables().length + 1) }, prevFocusableIndex() { return Math.max(0, this.focusables().indexOf(document.activeElement)) -1 }, autofocus() { let focusable = $el.querySelector('[autofocus]'); if (focusable) focusable.focus() }, scrollLocked: false, trigger: null, init() { this.$watch('show', this.toggleScroll.bind(this)) if (this.show) { this.lockScroll() } }, toggleScroll(value) { if (value) { this.trigger = document.activeElement; this.lockScroll(); return } this.unlockScroll() if (this.trigger) { this.trigger.focus() } this.trigger = null }, lockScroll() { if (this.scrollLocked) { return } this.scrollLocked = true window.controlUiKitScrollLock = window.controlUiKitScrollLock ?? { depth: 0, overflow: '', paddingRight: '' } let store = window.controlUiKitScrollLock let root = document.documentElement store.depth++ if (store.depth !== 1) { return } let gutter = Math.max(0, window.innerWidth - root.clientWidth) store.overflow = root.style.overflow store.paddingRight = root.style.paddingRight root.style.overflow = 'hidden' if (gutter !== 0) { root.style.paddingRight = gutter + 'px' } }, unlockScroll() { if (! this.scrollLocked) { return } this.scrollLocked = false let store = window.controlUiKitScrollLock if (! store) { return } store.depth = Math.max(0, store.depth - 1) if (store.depth !== 0) { return } let root = document.documentElement root.style.overflow = store.overflow root.style.paddingRight = store.paddingRight }, destroy() { this.unlockScroll() }, detail: { type: 'default', }, maxWidth: 'sm:max-w-2xl', openModal() { this.show = true this.loading = false this.maxWidth = this.width(this.maxWidth) }, submitAction(form) { this.loading = true let data = new FormData(form) fetch(form.action, { method: form.method, body: data, headers: { 'X-Requested-With': 'XMLHttpRequest' } }) .then(response => { if (! response.ok) throw response return response.json() }) .then(result => { this.show = false this.loading = false window.dispatchEvent(new CustomEvent('open-modal', { detail: { id: 'results', type: result.type ?? 'success', title: result.title ?? result.message ?? '', content: result.content ?? '' } })) }) .catch(() => { this.show = false this.loading = false window.dispatchEvent(new CustomEvent('open-modal', { detail: { id: 'results', type: 'danger' } })) }) }, width(maxWidth) { if (String(maxWidth).startsWith('sm:max-w-')) { return maxWidth } switch (maxWidth) { case 'sm': return 'sm:max-w-sm'; case 'md': return 'sm:max-w-md'; case 'lg': return 'sm:max-w-lg'; case '2xl': return 'sm:max-w-2xl'; case '3xl': return 'sm:max-w-3xl'; case '4xl': return 'sm:max-w-4xl'; case 'xl': default: return 'sm:max-w-xl'; } } }" x-init="$watch('show', value => value && setTimeout(autofocus, 50))" x-on:close.stop="show = false" x-on:keydown.escape.window="show = false" x-on:keydown.tab.prevent="$event.shiftKey || nextFocusable().focus()" x-on:keydown.shift.tab.prevent="prevFocusable().focus()" x-show="show" id="" class="overlay" style="display: none;">
                <div x-show="show" class="fixed inset-0 transform transition-all" x-on:click="show = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
                    <div class="absolute inset-0 bg-modal-blur opacity-75"></div>
                </div>
                <div x-show="show" class="panel scroll-body" :class="{ [maxWidth]: true }" role="dialog" aria-modal="true" tabindex="-1" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
                    <form method="POST" action="/items/1" class="form" x-on:submit.prevent="submitAction($event.target)">
                        <input type="hidden" name="_token" value="" autocomplete="off">
                        <div class="body text-sm leading-6"></div>
                        <div class="footer">
                            <button class="background default-background border default-border color default-color cursor font other padding rounded shadow min-w-20 space-x-0!" x-bind:disabled="loading" type="submit"> <span x-show="!loading">buttons.yes</span> <span x-show="loading">buttons.confirming</span> </button>
    ...
    public function an_action_modal_can_be_rendered_with_ajax_and_fields(): void
    {
        ...
        $expected = <<<'HTML'
            <div x-data="{ show: false, loading: false, focusables() { // All focusable element types... let selector = 'a, button, input, textarea, select, details, [tabindex]:not([tabindex=\'-1\'])' return [...$el.querySelectorAll(selector)] // All non-disabled elements... .filter(el =>
                ! el.hasAttribute('disabled')) }, firstFocusable() { return this.focusables()[0] }, lastFocusable() { return this.focusables().slice(-1)[0] }, nextFocusable() { return this.focusables()[this.nextFocusableIndex()] || this.firstFocusable() }, prevFocusable() { return this.focusables()[this.prevFocusableIndex()] || this.lastFocusable() }, nextFocusableIndex() { return (this.focusables().indexOf(document.activeElement) + 1) % (this.focusables().length + 1) }, prevFocusableIndex() { return Math.max(0, this.focusables().indexOf(document.activeElement)) -1 }, autofocus() { let focusable = $el.querySelector('[autofocus]'); if (focusable) focusable.focus() }, scrollLocked: false, trigger: null, init() { this.$watch('show', this.toggleScroll.bind(this)) if (this.show) { this.lockScroll() } }, toggleScroll(value) { if (value) { this.trigger = document.activeElement; this.lockScroll(); return } this.unlockScroll() if (this.trigger) { this.trigger.focus() } this.trigger = null }, lockScroll() { if (this.scrollLocked) { return } this.scrollLocked = true window.controlUiKitScrollLock = window.controlUiKitScrollLock ?? { depth: 0, overflow: '', paddingRight: '' } let store = window.controlUiKitScrollLock let root = document.documentElement store.depth++ if (store.depth !== 1) { return } let gutter = Math.max(0, window.innerWidth - root.clientWidth) store.overflow = root.style.overflow store.paddingRight = root.style.paddingRight root.style.overflow = 'hidden' if (gutter !== 0) { root.style.paddingRight = gutter + 'px' } }, unlockScroll() { if (! this.scrollLocked) { return } this.scrollLocked = false let store = window.controlUiKitScrollLock if (! store) { return } store.depth = Math.max(0, store.depth - 1) if (store.depth !== 0) { return } let root = document.documentElement root.style.overflow = store.overflow root.style.paddingRight = store.paddingRight }, destroy() { this.unlockScroll() }, detail: { type: 'default', }, maxWidth: 'sm:max-w-2xl', openModal() { this.show = true this.loading = false this.maxWidth = this.width(this.maxWidth) }, submitAction(form) { this.loading = true let data = new FormData(form) let allowed = ["name","email"] let filtered = new FormData() for (let key of allowed) { if (data.has(key)) filtered.append(key, data.get(key)) } data = filtered fetch(form.action, { method: form.method, body: data, headers: { 'X-Requested-With': 'XMLHttpRequest' } }) .then(response => { if (! response.ok) throw response return response.json() }) .then(result => { this.show = false this.loading = false window.dispatchEvent(new CustomEvent('open-modal', { detail: { id: 'results', type: result.type ?? 'success', title: result.title ?? result.message ?? '', content: result.content ?? '' } })) }) .catch(() => { this.show = false this.loading = false window.dispatchEvent(new CustomEvent('open-modal', { detail: { id: 'results', type: 'danger' } })) }) }, width(maxWidth) { if (String(maxWidth).startsWith('sm:max-w-')) { return maxWidth } switch (maxWidth) { case 'sm': return 'sm:max-w-sm'; case 'md': return 'sm:max-w-md'; case 'lg': return 'sm:max-w-lg'; case '2xl': return 'sm:max-w-2xl'; case '3xl': return 'sm:max-w-3xl'; case '4xl': return 'sm:max-w-4xl'; case 'xl': default: return 'sm:max-w-xl'; } } }" x-init="$watch('show', value => value && setTimeout(autofocus, 50))" x-on:close.stop="show = false" x-on:keydown.escape.window="show = false" x-on:keydown.tab.prevent="$event.shiftKey || nextFocusable().focus()" x-on:keydown.shift.tab.prevent="prevFocusable().focus()" x-show="show" id="" class="overlay" style="display: none;">
                <div x-show="show" class="fixed inset-0 transform transition-all" x-on:click="show = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
                    <div class="absolute inset-0 bg-modal-blur opacity-75"></div>
                </div>
                <div x-show="show" class="panel scroll-body" :class="{ [maxWidth]: true }" role="dialog" aria-modal="true" tabindex="-1" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
    ...
    public function an_action_modal_can_be_rendered_with_custom_id(): void
    {
        ...
        $expected = <<<'HTML'
            <div x-data="{ show: false, loading: false, focusables() { // All focusable element types... let selector = 'a, button, input, textarea, select, details, [tabindex]:not([tabindex=\'-1\'])' return [...$el.querySelectorAll(selector)] // All non-disabled elements... .filter(el =>
                ! el.hasAttribute('disabled')) }, firstFocusable() { return this.focusables()[0] }, lastFocusable() { return this.focusables().slice(-1)[0] }, nextFocusable() { return this.focusables()[this.nextFocusableIndex()] || this.firstFocusable() }, prevFocusable() { return this.focusables()[this.prevFocusableIndex()] || this.lastFocusable() }, nextFocusableIndex() { return (this.focusables().indexOf(document.activeElement) + 1) % (this.focusables().length + 1) }, prevFocusableIndex() { return Math.max(0, this.focusables().indexOf(document.activeElement)) -1 }, autofocus() { let focusable = $el.querySelector('[autofocus]'); if (focusable) focusable.focus() }, scrollLocked: false, trigger: null, init() { this.$watch('show', this.toggleScroll.bind(this)) if (this.show) { this.lockScroll() } }, toggleScroll(value) { if (value) { this.trigger = document.activeElement; this.lockScroll(); return } this.unlockScroll() if (this.trigger) { this.trigger.focus() } this.trigger = null }, lockScroll() { if (this.scrollLocked) { return } this.scrollLocked = true window.controlUiKitScrollLock = window.controlUiKitScrollLock ?? { depth: 0, overflow: '', paddingRight: '' } let store = window.controlUiKitScrollLock let root = document.documentElement store.depth++ if (store.depth !== 1) { return } let gutter = Math.max(0, window.innerWidth - root.clientWidth) store.overflow = root.style.overflow store.paddingRight = root.style.paddingRight root.style.overflow = 'hidden' if (gutter !== 0) { root.style.paddingRight = gutter + 'px' } }, unlockScroll() { if (! this.scrollLocked) { return } this.scrollLocked = false let store = window.controlUiKitScrollLock if (! store) { return } store.depth = Math.max(0, store.depth - 1) if (store.depth !== 0) { return } let root = document.documentElement root.style.overflow = store.overflow root.style.paddingRight = store.paddingRight }, destroy() { this.unlockScroll() }, detail: { type: 'default', }, maxWidth: 'sm:max-w-2xl', openModal() { this.show = true this.loading = false this.maxWidth = this.width(this.maxWidth) }, width(maxWidth) { if (String(maxWidth).startsWith('sm:max-w-')) { return maxWidth } switch (maxWidth) { case 'sm': return 'sm:max-w-sm'; case 'md': return 'sm:max-w-md'; case 'lg': return 'sm:max-w-lg'; case '2xl': return 'sm:max-w-2xl'; case '3xl': return 'sm:max-w-3xl'; case '4xl': return 'sm:max-w-4xl'; case 'xl': default: return 'sm:max-w-xl'; } } }" x-init="$watch('show', value => value && setTimeout(autofocus, 50))" x-on:close.stop="show = false" x-on:keydown.escape.window="show = false" x-on:keydown.tab.prevent="$event.shiftKey || nextFocusable().focus()" x-on:keydown.shift.tab.prevent="prevFocusable().focus()" x-show="show" id="delete-modal" class="overlay" style="display: none;">
                <div x-show="show" class="fixed inset-0 transform transition-all" x-on:click="show = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
                    <div class="absolute inset-0 bg-modal-blur opacity-75"></div>
                </div>
                <div x-show="show" class="panel scroll-body" :class="{ [maxWidth]: true }" role="dialog" aria-modal="true" tabindex="-1" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
    ...
    public function an_action_modal_can_be_rendered_with_a_type(): void
    {
        ...
        $expected = <<<'HTML'
            <div x-data="{ show: false, loading: false, focusables() { // All focusable element types... let selector = 'a, button, input, textarea, select, details, [tabindex]:not([tabindex=\'-1\'])' return [...$el.querySelectorAll(selector)] // All non-disabled elements... .filter(el =>
                ! el.hasAttribute('disabled')) }, firstFocusable() { return this.focusables()[0] }, lastFocusable() { return this.focusables().slice(-1)[0] }, nextFocusable() { return this.focusables()[this.nextFocusableIndex()] || this.firstFocusable() }, prevFocusable() { return this.focusables()[this.prevFocusableIndex()] || this.lastFocusable() }, nextFocusableIndex() { return (this.focusables().indexOf(document.activeElement) + 1) % (this.focusables().length + 1) }, prevFocusableIndex() { return Math.max(0, this.focusables().indexOf(document.activeElement)) -1 }, autofocus() { let focusable = $el.querySelector('[autofocus]'); if (focusable) focusable.focus() }, scrollLocked: false, trigger: null, init() { this.$watch('show', this.toggleScroll.bind(this)) if (this.show) { this.lockScroll() } }, toggleScroll(value) { if (value) { this.trigger = document.activeElement; this.lockScroll(); return } this.unlockScroll() if (this.trigger) { this.trigger.focus() } this.trigger = null }, lockScroll() { if (this.scrollLocked) { return } this.scrollLocked = true window.controlUiKitScrollLock = window.controlUiKitScrollLock ?? { depth: 0, overflow: '', paddingRight: '' } let store = window.controlUiKitScrollLock let root = document.documentElement store.depth++ if (store.depth !== 1) { return } let gutter = Math.max(0, window.innerWidth - root.clientWidth) store.overflow = root.style.overflow store.paddingRight = root.style.paddingRight root.style.overflow = 'hidden' if (gutter !== 0) { root.style.paddingRight = gutter + 'px' } }, unlockScroll() { if (! this.scrollLocked) { return } this.scrollLocked = false let store = window.controlUiKitScrollLock if (! store) { return } store.depth = Math.max(0, store.depth - 1) if (store.depth !== 0) { return } let root = document.documentElement root.style.overflow = store.overflow root.style.paddingRight = store.paddingRight }, destroy() { this.unlockScroll() }, detail: { type: 'danger', }, maxWidth: 'sm:max-w-2xl', openModal() { this.show = true this.loading = false this.maxWidth = this.width(this.maxWidth) }, width(maxWidth) { if (String(maxWidth).startsWith('sm:max-w-')) { return maxWidth } switch (maxWidth) { case 'sm': return 'sm:max-w-sm'; case 'md': return 'sm:max-w-md'; case 'lg': return 'sm:max-w-lg'; case '2xl': return 'sm:max-w-2xl'; case '3xl': return 'sm:max-w-3xl'; case '4xl': return 'sm:max-w-4xl'; case 'xl': default: return 'sm:max-w-xl'; } } }" x-init="$watch('show', value => value && setTimeout(autofocus, 50))" x-on:close.stop="show = false" x-on:keydown.escape.window="show = false" x-on:keydown.tab.prevent="$event.shiftKey || nextFocusable().focus()" x-on:keydown.shift.tab.prevent="prevFocusable().focus()" x-show="show" id="" class="overlay" style="display: none;">
                <div x-show="show" class="fixed inset-0 transform transition-all" x-on:click="show = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
                    <div class="absolute inset-0 bg-modal-blur opacity-75"></div>
                </div>
                <div x-show="show" class="panel scroll-body" :class="{ [maxWidth]: true }" role="dialog" aria-modal="true" tabindex="-1" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
    ...
    #[Test]
    public function an_action_modal_can_be_rendered_with_an_inline_title_class(): void
    {
        $this->setUpAlertConfig();

        $template = <<<'HTML'
            <x-modal-action route="/items/1" results-modal="results" title-class="my-title">
                <x-slot name="title">Are you sure?</x-slot>
            </x-modal-action>
            HTML;
    ...
    #[Test]
    public function an_action_modal_throws_an_exception_when_given_an_invalid_scroll(): void
    {
        ...
        new Action(scroll: 'bottom');
    }

    /**
     * `scroll` names a theme key suffix rather than a class, so unlike a style attribute it has
     * no `none`: `UseThemeFile::style()` resolves the string `none` to an empty string, which is
     * why the third mode is called `clip`.
     *
     * Note there is no counterpart here to `ModalTest`'s invalid-theme-value test: `x-modal-action`
     * defaults `scroll` to `body` in the constructor rather than passing `null`, so `style()`
     * returns the argument untouched and never reads `themes.default.modal.scroll` at all.
     */
    #[Test]
    public function an_action_modal_throws_an_exception_when_given_a_scroll_of_none(): void
    {
        $this->expectException(ControlUIKitException::class);
        $this->expectExceptionMessage('Modal scroll [] is invalid, please use one of [panel, body, clip]');

        new Action(scroll: 'none');
    }
